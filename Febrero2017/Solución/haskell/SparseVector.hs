...
vector n x = if n<0 then error "Tamaño no válido" else V (2^n) (Unif x)
...
get i (V _ (Unif x)) = x
get i (V sz (Node l r))
  | i < 0 || i > sz = error "indice fuera de rango"
  | i < (div sz 2) = get i (V (div sz 2) l)
  | otherwise = get (i - (div sz 2)) (V (div sz 2) r)
...
set i e (V sz arbol) = V sz (setEnTree i e sz arbol) 
  where
    setEnTree pos e 1 (Unif x) = (Unif e)
    setEnTree pos e sz (Unif x) = setEnTree pos e sz (Node (Unif x) (Unif x))
    setEnTree pos e sz (Node l r)
      | pos < 0 || pos > sz = error "indice fuera de rango"
      | pos < mitad = simplify (setEnTree pos e mitad l) r
      | otherwise = simplify l (setEnTree (pos - mitad) e mitad r)
        where mitad = (div sz 2)
...
-- Fill the table below (Seguramente este mal):
--
...
mapVector f (V n t) = V n (mapTree f t)
  where
...
filterVector:: (a -> Bool) -> Vector a -> Vector Bool
filterVector f (V n t) = (V n (filterTree f t))
   where
       filterTree f (Unif x) = (Unif (f x))
       filterTree f (Node lt rt) = (Node (filterTree f lt) (filterTree f rt))

-------------------------------------------------------------------------------
-- | Exercise i.

depthOf:: Int -> Vector a -> Int
depthOf i (V n t) =  prof i n t 0
   where
    prof i tam (Unif x) profundidad = profundidad
    prof i tam (Node lt rt) profundidad
      | i< mitad = prof i mitad lt (profundidad+1)
      | i>= mitad = prof (i-mitad) mitad rt (profundidad+1)
         where 
           mitad = div tam 2
...
instance (Eq a, Arbitrary a) => Arbitrary (Vector a) where
  arbitrary = do
    ...
    return $ foldr setEnTree ((vector :: Int -> a -> Vector a) sz x) (zip is xs)
    where
      setEnTree :: Eq a => (Int, a) -> Vector a -> Vector a
      setEnTree = uncurry set
