...
weights [] = D.empty 
weights lista = aux lista D.empty
  where
    aux [] dictaux = dictaux
    aux (x:xs) dictaux
      | D.isDefinedAt x dictaux = aux xs (D.insert x (fromJust(D.valueOf x dictaux) + 1) dictaux)
      | otherwise = aux xs (D.insert x 1 dictaux)
...
huffmanLeaves word = aux (D.keysAndValues (weights word)) PQ.empty
  where
    aux [] cola = cola
    aux ((k,v):xs) cola = aux xs (PQ.enqueue (WLeaf k v) cola)
...
huffmanTree word
  | length (D.keys(weights word)) < 2 = error "cadena con menos de dos caracteres distintos"
  | otherwise = aux (huffmanLeaves word)
    where
      aux arbol
        | PQ.isEmpty (PQ.dequeue arbol) = PQ.first arbol
        | otherwise = aux (PQ.enqueue mezcla colaactualizada)
            where
              first = PQ.first arbol
              second = PQ.first (PQ.dequeue arbol)
              mezcla = merge first second
              colaactualizada = PQ.dequeue (PQ.dequeue arbol)
...
joinDics dict1 dict2 = aux (D.keysAndValues dict1) dict2
  where
    aux [] dict = dict
    aux ((k,v):xs) dict = aux xs (D.insert k v dict) 
...
prefixWith x dict = aux x (D.keysAndValues dict) dict
  where
    aux x [] dict = dict
    aux x ((k,v):xs) dict = aux x xs (D.insert k ([x] ++ v) dict) 
...
huffmanCode (WLeaf k v) = (D.insert k [] D.empty)
huffmanCode (WNode l r _) = joinDics (prefixWith 0 (huffmanCode l)) (prefixWith 1 (huffmanCode r))
